...
//        [--frames <frames.json>]  sequence mode (a4 lab-main.ts): frames.json = [{t, yaw, pitch, roll, x, y, z,
//        scale (radians / W), idle, blink, blinkL, blinkR, expr}, ...]; each is rendered in the same page with
//        window.__frame(spec) and written to <out without .png>-000.png, -001.png, ...
...
const prof = resolve(args.prof || '/private/tmp/claude-501/-Users-onuroztaskiran-FE-Apps-tt6-gbolanding/ec762eb5-0e84-43b6-a8c2-a6f1c54d7112/scratchpad/hero-face/planb/chrome-prof-v002');
...
async function main() {
  ...
  const seq = [];
  if (ok && args.frames) {
    const frames = JSON.parse(readFileSync(resolve(args.frames), 'utf8'));
    for (let k = 0; k < frames.length; k++) {
      const r = await s('Runtime.evaluate', { expression: `window.__frame(${JSON.stringify(frames[k])})`, returnByValue: true, awaitPromise: true });
      const f = out.replace(/\.png$/, '') + '-' + String(k).padStart(3, '0') + '.png';
      writeFileSync(f, Buffer.from((r.result.value || '').replace(/^data:image\/png;base64,/, ''), 'base64'));
      seq.push(f);
    }
  }
  console.log(JSON.stringify({ ok, out: b64 ? out : null, ms: Date.now() - t0, info: JSON.parse(info.result.value || '{}'), seq, logs }, null, 1));
  ws.close(); kill();
  process.exit(ok ? 0 : 2);
}
